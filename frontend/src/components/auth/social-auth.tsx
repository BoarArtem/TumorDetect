...
export function SocialAuth({ label = "Sign in" }: { label?: string }) {
  const router = useRouter()

  const handle = (provider: string) => {
    toast.success(`${label} with ${provider}`, {
      description: "Demo mode — routing you to the dashboard.",
    })
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <Button variant="outline" className="h-10" onClick={() => handle("Google")}>
        <GoogleIcon />
        Google
      </Button>
      <Button variant="outline" className="h-10" onClick={() => handle("Microsoft")}>
        <MicrosoftIcon />
        Microsoft
      </Button>
    </div>
